...
can be a referenced object, a related collection, or optional dictionary
metadata. Nothing is expanded by default.
...
# Include every amenity-specific choice in the dictionary
...
For the amenity dictionary, `expand=options` adds the complete localized
`options` array to entries whose `detailKinds` include `options`. The option
list is omitted without the expand. `detailKinds` itself is always returned, so
clients can see whether an amenity supports options before requesting them.
