...
1. **The amenity dictionary** — the list of every _possible_ amenity, each with a
   ...
   from [`GET /api/v2/amenities`](/api/v2/reference/get-amenities).
...
<Card title="GET /api/v2/amenities" icon="square-terminal" href="/api/v2/reference/get-amenities" horizontal>
...
reference list, not paginated. Each entry:

| Field          | Meaning                                                                                                   |
| -------------- | --------------------------------------------------------------------------------------------------------- |
| `code`         | Identifier in `category.slug` form (e.g. `wellness.sauna`). Use it to filter and to match values.           |
| `category`     | The category the amenity belongs to (e.g. `wellness`, `parking`, `connectivity`).                          |
| `scope`        | The levels it applies to — `["hotel"]`, `["roomType"]`, or `["hotel", "roomType"]`.                         |
| `valueType`    | `boolean` (the amenity is simply present) or `count` (has a `quantity`, e.g. parking spaces).               |
...
**has**. Each item references a dictionary `code`, optionally with extra detail:
...
| `condition` | Set only when availability is limited — `seasonal`, `dayOfWeek`, or `underConstruction`. Omitted otherwise.  |
...
Both [`GET /api/v2/hotels`](/api/v2/reference/get-hotels) and
[`GET /api/v2/room-types`](/api/v2/reference/get-room-types) accept two amenity filters:
