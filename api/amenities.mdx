...
3. To revalidate directly, repeat the same request — with the same
   `Accept-Language` and `expand` — and add `If-None-Match: <cached ETag>`. The
   API returns `304` with no body while your copy is current.
...
differs per requested locale set and `expand=options`; the version header does
not.
