...
key and return `null` when its translation is missing, except image `alt`,
which falls back to the localized hotel name.
...
and remains available throughout API v2.
...
an available variant for `src`, falling back to `url`. Use the returned URLs
directly; do not construct URLs for missing widths. Images are never enlarged,
so an 1100-pixel original has only `w480` and `w960`.
