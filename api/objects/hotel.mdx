...
parent category, and supported detail fields. Add `expand=options` when you
also need every allowed amenity-specific option. See
[Amenity value](/api/objects/amenity-value) for the exact `detailKinds` to
response-field mapping. Fields not supported by an amenity are omitted.
...
of nights. Applicable public-channel promotions are reflected in these totals;
the corresponding `*Original` value is present when a promotion lowers the
price. The cheapest room can differ by currency. `priceFrom` is `null` when the
hotel has no usable price; an unavailable currency key is omitted.
