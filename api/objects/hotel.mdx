...
a requested translation is missing. Other localized fields preserve the
requested key and return `null` when its translation is missing, except image
`alt`, which falls back to the localized hotel name.

## Images

Every image contains `url`, `thumbnailUrl`, `width`, `height`, `alt`, and
`variants`. Use `url` for the original or lightbox. `thumbnailUrl` is deprecated
and remains available throughout API v2. The [images guide](/api/images)
provides a complete rendering example.

Build `srcset` from the returned `variants` entries: `w480` means `480w`, and
likewise for `w960`, `w1440`, and `w1920`. Set `sizes` for your layout and choose
an available variant for `src`, followed by `thumbnailUrl` and then `url` as
fallbacks. Use the returned URLs directly; do not construct URLs for missing
widths. Images are never enlarged, so an 1100-pixel original has only `w480`
and `w960`.

Use non-null `width` and `height` as intrinsic HTML dimensions to reserve the
correct aspect ratio. They describe the original after orientation. When
dimensions are unavailable, both are `null` and `variants` is `{}`. An original
narrower than 480 pixels also has `{}`, but retains its known dimensions.

`alt` follows [`Accept-Language`](/api/localization). Missing or null image
translations fall back to the localized hotel name. Preserve an explicit empty
string: it marks a decorative image. A non-empty value is trimmed.
...
`priceFrom` holds the lowest prices the hotel currently sells, keyed by currency
(`CZK`, `EUR`). Under each currency you get the hotel-wide price `all` and,
when the hotel sells such stays, the categories `health` (spa cures) and
`wellness`. Each of them reports two independent minima, `perPersonPerNight`
and `perStay`, each with its own `amount`, `original`, `nights`, `occupancy`,
and `visitType`. `all` is always present when the currency is; a category is
omitted when the hotel sells nothing in it. `priceFrom` is `null` when the hotel
has no usable price, and a currency without a usable price is omitted. The
`visitType.name` falls back to the Czech value like the hotel `name`. See
[Prices](/api/prices) for the selection rules and an example.
