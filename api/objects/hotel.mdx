...
`amenities` groups the hotel's assigned structured amenities by category.
Categories without assigned amenities are omitted, and the response does not
contain the complete system dictionary. Each nested value's `code` references
the [amenity dictionary](/api/amenities) and carries applicable access,
distance, type, opening-hour, and localized note details.
...
parent category, and supported detail fields. See
...
`priceFrom` contains the indicative lowest price when one is available and is
`null` otherwise.
