---
title: "Amenity value"
description: "A structured amenity selected for a hotel or room type."
openapi-schema: "/api/v2/openapi.yaml AmenityValue"
---

An amenity assigned to a hotel or room type, returned under
`amenityCategories[].amenities[]`. Selected amenities are grouped by category, and categories without assigned amenities
...
`GET /api/v2/amenity-categories?scope=hotel` or `?scope=roomType` under `data[].amenities[]`. The parent
...
`category` is returned once on the parent amenity group, by `code`. The
amenity's `label` and `detailKinds` are dictionary metadata and are not repeated
inside each selected amenity value by default. Request
