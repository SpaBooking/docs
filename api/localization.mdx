...
Two fields are exceptions: the hotel `name` and the `visitType.name` inside
[`priceFrom`](/api/prices). A missing translation falls back to the Czech
value, so these values are never `null`.
