---
title: "API v2 changelog"
description: "Public changes to SpaPortal API v2."
...
- **Breaking (Draft room types):** `name` replaces `category`,
  `occupancy.standard` replaces `beds`, and `amenityCategories` replaces
  `facilities`. These changes also apply to room types embedded in hotels.
- **Breaking:** Replace the `beds` query filter with `occupancy`. Requests
  using `beds` now return `400`.
- Added `unitType`, `tier`, `bedConfiguration`, `size`, `renovationYear`,
  `rules`, and `building`. Enum labels are inline. Missing English and Russian
  generated names are composed in the response. Custom names keep their saved
  translations. The pet rule includes the hotel's policy.
- Added `unitType`, `tier`, `amenities`, and `amenityCategory` filters on room
  types. `expand` accepts `hotel` and `amenityLabels` together.
- Added `scope=roomType` to the amenity dictionary. The default `scope=hotel`
  response body is unchanged. `expand=roomTypes,amenityLabels` includes labels
  on embedded room amenities.
- The amenity version now covers both scopes and is also returned on room type
  endpoints. Its value changes once with this release; refresh both cached
  dictionaries. ETags now include the scope.
- Room types remain **Draft** until client integration is complete.
...
- `variants` contains available WebP URLs at widths 480, 960, 1440, and 1920.
  Sizes larger than the original are omitted. When no variant exists, fall back
  to `thumbnailUrl` and then `url`.
